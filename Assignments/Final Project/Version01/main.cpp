...
{
	int count = 0;
	Traveler* singleTraveler = static_cast<Traveler*>(arg);

	#if DEBUG_OUTPUT == ON
		std::cout << "\n + Traveler [" << singleTraveler->index << "] created " <<
			" still searching: "<< singleTraveler->searching << std::endl;
		for (unsigned long i=0; i<singleTraveler->segmentList.size(); i++)
		{
			std::cout << "\n\t(row=" << singleTraveler->segmentList[i].row << ", col=" <<
			singleTraveler->segmentList[i].col << "), direction: " << dirStr(singleTraveler->segmentList[i].dir) << std::endl;
		}
		std::cout << std::flush;
	#endif

	singleTraveler->searching = true;
	int moveAttempts = 0;
	while(singleTraveler->searching)
	{
		// move the traveler until the exit is reached
		bool couldMove = singleTraveler->move();
		if (couldMove)
		{
			moveAttempts = 0;
			TravelerSegment tail = singleTraveler->getTail();
			 count++;
			 if (count % growthRate == 0)
			 {
				singleTraveler->segmentList.push_back(tail);
			 }
		}
		else
		{
			if (moveAttempts++ > 20)
				singleTraveler->searching = false;
		}
		usleep(sleepTime);
	}
	return NULL;
...
bool Traveler::move(void)
{
	...
	vector<Direction> freeSpaces = getFreeSpaces(getHead());
	if (freeSpaces.empty())
	{
		canMove = false;
		#if DEBUG_OUTPUT == ON
...
void quit (void)
{
	travelerList[0].searching = false;
	cleanGrid();
	exit(0);
}
